...
import { DEPENDENCY_STRATEGIES } from './config.js'
import { getDynamicDependencyStrategies } from './version-resolver.js'

/**
 * Get dependencies based on strategy
 * @param {Object} options - CLI options
 * @param {boolean} useDynamic - Whether to fetch latest versions from npm
 * @returns {Promise<Object>|Object} Dependencies object
 */
export function getDependencies(options, useDynamic = false) {
  let strategy = 'stable' // Default
  ...
  if (useDynamic && strategy === 'stable') {
    ...
    return getDynamicDependencyStrategies().then(
...
export function validateProjectName(name) {
  ...
  if (name && name.startsWith('-')) {
    errors.push('Project name cannot start with a hyphen')
  }
  return errors
}
 ...
 */
export async function displayProjectSummary(name, projectPath, options) {
 ...
 */
export function displaySuccessInstructions(projectName, wasInstalled = false) {
  if (wasInstalled) {
